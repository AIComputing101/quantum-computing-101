...
def visualize_bb84_protocol():
    ...
    ax2.plot(eavesdrop_levels * 100, error_rates * 100, 'b-o', linewidth=2, markersize=6)
    ax2.axhline(y=11, color='red', linestyle='--', linewidth=2, 
    ...
    efficiency_no_eve = [q * 0.5 * 0.95 for q in total_qubits]  # 50% basis match, 95% kept
    efficiency_with_eve = [q * 0.5 * 0.6 for q in total_qubits]  # More discarded due to errors
    ...
    for i, step in enumerate(steps):
        y_pos = 9 - i
        ax4.text(0.5, y_pos, step, fontsize=11, 
                bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.7))
    
    ax4.set_title("BB84 Protocol Steps", fontsize=14, fontweight='bold')
    
    plt.suptitle('BB84 Quantum Key Distribution Protocol', 
...
def main():
    ...
    parser.add_argument('--key-length', type=int, default=20,
                       help='Number of qubits to use in protocol')
    parser.add_argument('--skip-visualization', action='store_true',
                       help='Skip the visualization plots')
    parser.add_argument('--show-eavesdropping', action='store_true',
