...
class SupplyChainNetwork:
    ...
    def add_facility(self, facility_id, facility_type, location, capacity, fixed_cost=0):
    ...
    def add_transportation_link(self, from_facility, to_facility, 
                              transport_cost, transit_time, capacity=float('inf')):
...
class VehicleRoutingOptimizer:
    ...
    def add_vehicle(self, vehicle_id, capacity, cost_per_km, fixed_cost=0, 
                   vehicle_type='truck', depot=None):
    ...
    def create_vrp_qubo(self, customers, depot, max_vehicles=4):
        ...
        for i, loc1 in enumerate(all_locations):
            for j, loc2 in enumerate(all_locations):
                if i != j:
                    if loc1 in self.supply_chain.facilities and loc2 in self.supply_chain.facilities:
    ...
    def run_qaoa_vrp(self, hamiltonian, n_layers=2, max_iter=50):
        ...
        selected_customers = [i for i, bit in enumerate(route_bitstring[::-1]) if bit == '1']
...
class SupplyChainAnalyzer:
    ...
    def analyze_network_efficiency(self, supply_chain):
        ...
        if 'betweenness' in metrics['centrality']:
            avg_betweenness = np.mean(list(metrics['centrality']['betweenness'].values()))
    ...
    def calculate_performance_kpis(self, supply_chain, vrp_results, inventory_results):
        ...
        # Cost efficiency
        total_transport_cost = sum(edge['transport_cost'] * edge['utilization'] 
                                 for edge in supply_chain.edges.values())
        total_fixed_cost = sum(facility['fixed_cost'] 
                             for facility in supply_chain.facilities.values())
        total_inventory_cost = 0
        
        if inventory_results and 'quantum_solution' in inventory_results:
            total_inventory_cost = abs(inventory_results['quantum_solution']['optimal_cost'])
        
        kpis['cost_efficiency'] = {
            'total_cost': total_transport_cost + total_fixed_cost + total_inventory_cost,
            'transport_cost': total_transport_cost,
            'fixed_cost': total_fixed_cost,
            'inventory_cost': total_inventory_cost,
            'cost_per_unit': (total_transport_cost + total_inventory_cost) / max(1, len(supply_chain.demands))
        }
        
        # Service level
        fulfilled_demands = sum(1 for demand in supply_chain.demands.values() if demand['fulfilled'])
        ...
        # Asset utilization
        avg_facility_utilization = np.mean([facility['utilization'] 
                                          for facility in supply_chain.facilities.values()])
        avg_transport_utilization = np.mean([edge['utilization'] 
                                           for edge in supply_chain.edges.values()])
        ...
        kpis['asset_utilization'] = {
            ...
            'capacity_utilization': (avg_facility_utilization + avg_transport_utilization) / 2
        }
        
        # Sustainability metrics
        total_distance = sum(edge['distance'] * edge['utilization'] 
                           for edge in supply_chain.edges.values())
        ...
        kpis['resilience'] = {
            'network_redundancy': 1 - len(network_metrics['bottlenecks']) / max(1, network_metrics['n_nodes']),
            'supplier_diversity': len(set(facility['type'] for facility in supply_chain.facilities.values())),
    ...
    def generate_optimization_report(self, supply_chain, quantum_results, classical_comparison=None):
        ...
        if 'inventory' in quantum_results:
            ...
            report['optimization_results']['inventory_management'] = {
                ...
                'solution_probability': inventory_results['quantum_solution']['solution_probability'],
                'configuration_efficiency': inventory_results['quantum_solution']['success']
            }
        ...
        report['roi_analysis'] = {
            ...
            'net_present_value': quantum_optimization_savings * 5 - implementation_cost,  # 5-year NPV
            'roi_percentage': (quantum_optimization_savings / implementation_cost - 1) * 100
        }
...
def visualize_supply_chain_results(supply_chain, vrp_results, inventory_results, kpis):
    ...
    if pos:
        nx.draw(graph, pos, ax=ax1, node_color=colors, node_size=sizes, 
    ...
    if 'cost_efficiency' in kpis:
        ...
        costs = [cost_data['transport_cost'], cost_data['fixed_cost'], cost_data['inventory_cost']]
        labels = ['Transport', 'Fixed', 'Inventory']
        colors_pie = ['lightblue', 'lightgreen', 'lightyellow']
        
        wedges, texts, autotexts = ax2.pie(costs, labels=labels, colors=colors_pie, 
    ...
    if 'service_level' in kpis:
        ...
        bars = ax3.bar(metrics, values, color=['green', 'blue', 'orange', 'red'], alpha=0.7)
    ...
    if 'asset_utilization' in kpis:
        ...
        categories = ['Facilities', 'Transport', 'Inventory\nTurnover', 'Overall\nCapacity']
        utilizations = [
        ...
        bars = ax4.bar(categories, utilizations, color=['purple', 'brown', 'pink', 'cyan'], alpha=0.7)
    ...
    if 'sustainability' in kpis:
        ...
        values = [
            1 - min(sustain_data['carbon_footprint'] / 10000, 1),  # Normalize and invert
    ...
    if vrp_results:
        ...
        summary_text += f"• Selected customers: {len(vrp_results['selected_customers'])}\n"
        summary_text += f"• Solution probability: {vrp_results['route_probability']:.1%}\n"
    ...
    if inventory_results and 'quantum_solution' in inventory_results:
        ...
        summary_text += f"• Solution probability: {inv_sol['solution_probability']:.1%}\n"
...
def main():
    parser = argparse.ArgumentParser(description="Quantum Supply Chain and Logistics Optimization")
    parser.add_argument('--n-facilities', type=int, default=8,
                       help='Number of facilities in supply chain')
    parser.add_argument('--n-products', type=int, default=5,
                       help='Number of products')
    parser.add_argument('--n-vehicles', type=int, default=4,
                       help='Number of vehicles for routing')
    parser.add_argument('--qaoa-layers', type=int, default=2,
                       help='Number of QAOA layers')
    parser.add_argument('--max-iter', type=int, default=50,
                       help='Maximum optimization iterations')
    parser.add_argument('--vehicle-routing', action='store_true',
                       help='Run vehicle routing optimization')
    parser.add_argument('--inventory-optimization', action='store_true',
                       help='Run inventory optimization')
    parser.add_argument('--network-analysis', action='store_true',
                       help='Perform network analysis')
    parser.add_argument('--sustainability-focus', action='store_true',
    ...
    try:
            ...
            for j, to_facility in enumerate(facilities_created):
                if i != j and np.random.random() > 0.6:  # 40% connection probability
                    ...
                    supply_chain.add_transportation_link(
                        from_facility, to_facility, transport_cost, transit_time, capacity
                    )
        ...
        for customer in customers:
            for product_id in products:
                if np.random.random() > 0.5:  # 50% chance of demand
                    ...
                    supply_chain.add_demand(customer, product_id, quantity, due_date, priority)
        ...
        if args.vehicle_routing:
            ...
            # Create VRP QUBO
            depot = customers[0] if customers else facilities_created[0]
            target_customers = customers[1:min(8, len(customers))]  # Limit for quantum simulation
            
            if target_customers:
                hamiltonian, distance_matrix, customer_list = vrp_optimizer.create_vrp_qubo(
        ...
        if args.inventory_optimization:
            ...
            target_facilities = [f for f in facilities_created if 'warehouse' in f or 'distribution' in f]
            ...
            if target_products and target_facilities:
                ...
                print(f"     Configuration: {quantum_solution['inventory_configuration']}")
                print(f"     Solution probability: {quantum_solution['solution_probability']:.1%}")
        ...
        if args.network_analysis:
            ...
            if 'asset_utilization' in kpis:
                ...
                print(f"     Facility utilization: {util_data['facility_utilization']:.1%}")
                print(f"     Capacity utilization: {util_data['capacity_utilization']:.1%}")
            ...
            print(f"   Efficiency improvement: {exec_summary['efficiency_improvement']:.1f}%")
        ...
        if args.show_visualization and args.network_analysis:
            visualize_supply_chain_results(supply_chain, vrp_results, inventory_results, kpis)
        
        print(f"\n📚 Key Insights:")
        print(f"   • Quantum algorithms excel at complex constraint optimization")
        print(f"   • QAOA provides near-optimal solutions for VRP and inventory problems")
