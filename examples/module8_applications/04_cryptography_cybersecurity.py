...
class QuantumKeyDistribution:
    ...
    def bb84_protocol(self, eve_present=False, eve_intercept_rate=0.2):
        ...
        # Alice's preparation
        alice_bits = [secrets.randbelow(2) for _ in range(self.key_length * 2)]  # Double for sifting
        alice_bases = [secrets.randbelow(2) for _ in range(self.key_length * 2)]  # 0: rectilinear, 1: diagonal
        ...
        test_indices = secrets.SystemRandom().sample(range(len(sifted_alice_bits)), test_sample_size)
        ...
        quantum_bit_error_rate = error_count / test_sample_size if test_sample_size > 0 else 0
        ...
        self.security_metrics['bb84'] = {
            ...
            'sifting_efficiency': (len(sifted_alice_bits) + test_sample_size) / len(alice_bits),
        ...
        if self.verbose:
            print(f"     Sifted {len(sifted_alice_bits) + test_sample_size} bits from {len(alice_bits)}")
    ...
    def e91_protocol(self, entanglement_fidelity=0.95):
        ...
        bell_test_indices = [i for i in range(n_pairs) if 
                           alice_angles[i] != bob_angles[i]][:100]  # Sample for Bell test
        ...
        for idx in bell_test_indices:
            ...
            if len(bell_test_indices) >= 4:
                ...
                correlation = 1 if alice_measurements[idx] == bob_measurements[idx] else -1
                if abs(correlation) > 1/np.sqrt(2):  # Quantum threshold
                    bell_violations += 1
        
        bell_violation_rate = bell_violations / len(bell_test_indices) if bell_test_indices else 0
        
        # Key extraction from correlated measurements
        key_indices = [i for i in range(n_pairs) if 
                      alice_angles[i] == bob_angles[i]][:self.key_length]
        ...
        if self.verbose:
            ...
            print(f"     Entanglement verified: {self.security_metrics['e91']['entanglement_verified']}")
...
class PostQuantumCryptography:
    ...
    def lattice_based_encryption(self, message, n=512, q=8192):
        ...
        self.security_analysis['lattice'] = {
            ...
            'ciphertext_expansion': len(ciphertext_blocks) * (len(u) + 1) * np.log2(q) / len(message_bits)
        }
    ...
    def hash_based_signatures(self, message, tree_height=10):
        ...
        message_hash = hashlib.sha256(message.encode() if isinstance(message, str) else message).digest()
...
class QuantumSecurityAnalyzer:
    ...
    def analyze_classical_security(self, key_size, algorithm_type):
        ...
        classical_security = {
            'AES-128': {'key_size': 128, 'quantum_security': 64, 'classical_security': 128},
            'AES-256': {'key_size': 256, 'quantum_security': 128, 'classical_security': 256},
            'RSA-2048': {'key_size': 2048, 'quantum_security': 0, 'classical_security': 112},
            'RSA-3072': {'key_size': 3072, 'quantum_security': 0, 'classical_security': 128},
            'ECC-256': {'key_size': 256, 'quantum_security': 0, 'classical_security': 128},
            'ECC-384': {'key_size': 384, 'quantum_security': 0, 'classical_security': 192}
        }
        
        security_info = classical_security.get(algorithm_type, {
            'key_size': key_size,
        ...
        for alg_name, alg_data in pq_algorithms.items():
            if alg_name == 'lattice':
                ...
                assessment[alg_name] = {
                    ...
                    'security_level': min(256, int(np.log2(sieve_complexity**(0.75)))),
                ...
                assessment[alg_name] = {
                    ...
                    'quantum_complexity': isd_complexity**(0.5),  # Square root speedup
        ...
        if qkd_results:
            ...
            report['qkd_analysis'] = {
                'bb84_protocol': {
                    ...
                    'practical_limitations': ['Distance limited', 'Low key rates', 'Expensive infrastructure']
                },
                'e91_protocol': {
                    'security_level': 'Information-theoretic',
                    'entanglement_verification': e91_metrics.get('entanglement_verified', False),
                    'bell_violations': e91_metrics.get('bell_violation_rate', 0),
                    'practical_limitations': ['Requires stable entanglement', 'Technical complexity']
                },
                'overall_assessment': {
...
def visualize_cryptography_results(qkd_metrics, pq_security, security_report):
    ...
    if pq_security:
        ...
        bars = ax2.bar(algorithms, security_levels, color=colors[:len(algorithms)], alpha=0.7)
    ...
    bars1 = ax3.bar(x - width/2, classical_security, width, 
                   label='Classical Security', alpha=0.7, color='blue')
    bars2 = ax3.bar(x + width/2, quantum_security, width,
                   label='Post-Quantum Security', alpha=0.7, color='red')
    ...
    for bar, size in zip(bars, key_sizes):
        ...
        ax5.text(bar.get_x() + bar.get_width()/2., height,
                f'{size}b', ha='center', va='bottom', fontsize=8)
...
def main():
    parser = argparse.ArgumentParser(description="Quantum Cryptography and Cybersecurity")
    parser.add_argument('--qkd-protocol', choices=['bb84', 'e91', 'both'], 
                       default='bb84', help='QKD protocol to implement')
    parser.add_argument('--key-length', type=int, default=256,
                       help='Quantum key length in bits')
    parser.add_argument('--noise-level', type=float, default=0.05,
                       help='Quantum channel noise level')
    parser.add_argument('--eve-attack', action='store_true',
                       help='Simulate eavesdropping attack')
    parser.add_argument('--post-quantum', action='store_true',
                       help='Analyze post-quantum cryptography')
    parser.add_argument('--security-analysis', action='store_true',
                       help='Perform comprehensive security analysis')
    parser.add_argument('--migration-planning', action='store_true',
                       help='Generate migration strategy')
    ...
    try:
        ...
        if args.qkd_protocol in ['bb84', 'both']:
            ...
            if args.eve_attack:
                ...
                shared_key_eve = qkd_eve.bb84_protocol(eve_present=True, eve_intercept_rate=0.3)
        ...
        if args.qkd_protocol in ['e91', 'both']:
            ...
            if shared_key_e91:
                print(f"     ✅ Entanglement-based key established: {len(shared_key_e91)} bytes")
        ...
        if args.post_quantum:
            ...
            print(f"     Public key size: ~{len(lattice_result['public_key'][0]) * len(lattice_result['public_key'][0][0]) * 13 // 8} bytes")
            
            # Hash-based signatures
            print(f"\n   Hash-based Signatures (Merkle):")
            hash_result = pqc.hash_based_signatures(test_message, tree_height=8)
            
            print(f"     Signature components: {len(hash_result['signature']['ots_signature'])}")
            print(f"     Authentication path: {len(hash_result['signature']['auth_path'])} nodes")
        ...
        if args.security_analysis:
            ...
            for alg, params in classical_algorithms.items():
                ...
                print(f"       Classical security: {attack_analysis['time_to_break_classical']}")
                print(f"       Quantum attack: {attack_analysis['time_to_break_quantum']}")
            ...
            if pq_results:
                ...
                for alg, assessment in pq_assessment.items():
                    print(f"     {alg.title()}:")
                    print(f"       Security assumption: {assessment['security_assumption']}")
                    print(f"       Classical attack: {assessment['best_classical_attack']}")
            ...
            if args.migration_planning:
                ...
                security_report = analyzer.generate_security_report(
                    qkd_results, pq_assessment if pq_results else {}, classical_algorithms
                )
                
                print(f"\n   Executive Summary:")
                exec_summary = security_report['executive_summary']
                print(f"     Quantum threat timeline: {exec_summary['quantum_threat_timeline']}")
                print(f"     Current vulnerability: {exec_summary['current_vulnerability']}")
                ...
                print(f"     Phase 2 (Transition): {migration['phase_2_transition'][0]}")
                print(f"     Phase 3 (Full migration): {migration['phase_3_full_migration'][0]}")
        ...
        print(f"\n📚 Key Insights:")
        print(f"   • QKD provides information-theoretic security but limited practicality")
        print(f"   • Post-quantum algorithms offer quantum resistance with computational assumptions")
        print(f"   • Migration to quantum-safe cryptography is urgent and complex")
        print(f"   • Hybrid approaches balance security and performance during transition")
        ...
        print(f"   • Competitive advantage through early adoption of quantum-safe systems")
