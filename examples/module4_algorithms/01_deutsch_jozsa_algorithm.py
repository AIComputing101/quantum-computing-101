...
class DeutschJozsaOracle:
    ...
    def __init__(self, n_qubits, function_type="random"):
        ...
        print(
            f"🔮 Oracle created: {function_type} function with {n_qubits} input qubits"
        )
...
def implement_deutsch_jozsa_algorithm(oracle):
    ...
    zero_string = "0" * n
    if (
        zero_string in counts and counts[zero_string] > 512
    ):  # Threshold for quantum measurement
...
def demonstrate_quantum_advantage():
    ...
    print("-------|-----------------|-----------------------------")
    for n, q_queries, c_queries in zip(
        qubit_counts, quantum_queries, classical_queries
    ):
    ...
    ax1.plot(
        qubit_counts, quantum_queries, "b-o", label="Quantum", linewidth=3, markersize=8
    )
    ax1.plot(
        qubit_counts,
        classical_queries,
    ...
    for x, y in zip(qubit_counts, speedup):
        ax2.text(
            x, y + y * 0.1, f"{y:.0f}x", ha="center", va="bottom", fontweight="bold"
        )
...
def test_different_functions():
    ...
    fig, axes = plt.subplots(
        1, len(function_types), figsize=(5 * len(function_types), 4)
    )
...
def analyze_success_probability():
    ...
    ax.plot(
        shot_counts,
        success_rates["balanced"],
        "r-s",
        label="Balanced Function",
        linewidth=2,
        markersize=8,
    )
...
def main():
    ...
    parser.add_argument(
        "--qubits", type=int, default=3, help="Number of input qubits (default: 3)"
    )
    parser.add_argument(
        "--function-type",
        choices=["constant_0", "constant_1", "balanced", "random"],
        default="random",
        help="Type of function to test (default: random)",
    )
