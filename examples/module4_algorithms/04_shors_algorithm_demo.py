...
class ShorsAlgorithmDemo:
            ...
            if measured_s > 0:
                ...
                for r in range(1, 16):
                    if abs(fraction - round(fraction * r) / r) < 0.1:
                        return {'period_estimate': r, 'circuit': circuit, 'counts': counts}
    ...
    def visualize_results(self, factorization_result, period_result=None):
        ...
        if 'factors' in factorization_result:
            ...
            verification = f"Verification: {factors[0]} × {factors[1]} = {np.prod(factors)}"
            ax1.text(0.5, max(factors) * 0.8, verification, 
                    transform=ax1.transData, ha='center', 
                    bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        else:
            ax1.text(0.5, 0.5, f"Error: {factorization_result.get('error', 'Unknown')}", 
...
def main():
    ...
    parser.add_argument('--quantum-demo', action='store_true', help='Run quantum period finding demo')
    ...
    try:
        ...
        if 'factors' in result:
            ...
            print(f"✅ Successfully factored {args.number} = {factors[0]} × {factors[1]}")
        else:
        ...
        print(f"• Real implementation requires sophisticated modular arithmetic circuits")
