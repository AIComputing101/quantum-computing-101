...
class GroverSearchAlgorithm:
    ...
    def simulate_grover_amplitude_evolution(self, n_qubits, marked_items, max_iterations=None):
        """Simulate amplitude evolution throughout Grover iterations."""
        if max_iterations is None:
            max_iterations = 2 * self.calculate_optimal_iterations(n_qubits, len(marked_items))
        ...
        for iteration in range(max_iterations):
            ...
            state = Statevector.from_instruction(circuit_no_measure)
            self._record_amplitude_data(state, marked_items, iteration + 1, evolution_data)
    ...
    def _record_amplitude_data(self, state, marked_items, iteration, evolution_data):
        ...
        unmarked_amp = np.mean([abs(state_vector[i]) for i in unmarked_indices]) if unmarked_indices else 0
    ...
    def execute_search(self, shots=1024):
        ...
        theoretical_prob = self._calculate_theoretical_success_probability(N, M, self.iterations_used)
    ...
    def compare_with_classical(self, target_items, search_space_size):
        ...
        comparison['advantage'] = {
            'speedup_vs_average': classical_average_case / quantum_queries if quantum_queries > 0 else 1,
            'speedup_vs_worst': classical_worst_case / quantum_queries if quantum_queries > 0 else 1,
    ...
    def visualize_results(self, evolution_data=None, comparison_data=None):
        ...
        if evolution_data:
            ...
            axes[0, 0].plot(iterations, evolution_data['marked_probability'], 'r-o', 
                           label='Marked Items', linewidth=2, markersize=4)
            axes[0, 0].plot(iterations, evolution_data['unmarked_probability'], 'b-s', 
                           label='Unmarked Items', linewidth=2, markersize=4)
        ...
        if evolution_data:
            axes[0, 1].plot(iterations, evolution_data['marked_probability'], 'g-o', linewidth=2)
            ...
            axes[0, 1].axvline(x=optimal_iter, color='red', linestyle='--', alpha=0.7, 
        ...
        if comparison_data:
            ...
            for bar, value in zip(bars, queries):
                ...
                axes[1, 1].text(bar.get_x() + bar.get_width()/2., height + max(queries)*0.01,
                               f'{value:.1f}', ha='center', va='bottom')
            
            # Add speedup text
            speedup = comparison_data['advantage']['speedup_vs_average']
            axes[1, 1].text(0.5, 0.95, f'Quantum Speedup: {speedup:.1f}x', 
    ...
    def generate_summary_report(self):
        ...
        if hasattr(self, 'search_results'):
            ...
            print(f"   Marked items: {len(results['marked_items'])} {results['marked_items']}")
            print(f"   Iterations used: {results['iterations_used']}")
            print(f"   Success probability: {results['success_probability']:.4f}")
            print(f"   Theoretical probability: {results['theoretical_probability']:.4f}")
...
Examples:
  ...
  python 02_grovers_search_algorithm.py --show-evolution --show-visualization
    ...
    parser.add_argument('--marked-items', type=int, nargs='+', default=[3],
    ...
    try:
        ...
        # Build circuit
        circuit = grover.build_grover_circuit(args.qubits, args.marked_items, args.iterations)
        ...
        if args.show_evolution:
            ...
            evolution_data = grover.simulate_grover_amplitude_evolution(args.qubits, args.marked_items)
        
        # Classical comparison
        comparison_data = None
        if args.compare_classical:
            comparison_data = grover.compare_with_classical(args.marked_items, search_space_size)
            
            print(f"\nClassical vs Quantum Comparison:")
            print(f"   Classical average queries: {comparison_data['classical']['average_case_queries']:.1f}")
            print(f"   Quantum queries: {comparison_data['quantum']['queries']}")
            print(f"   Speedup: {comparison_data['advantage']['speedup_vs_average']:.1f}x")
