...
def demonstrate_eigenvalue_problems():
    ...
    for name, matrix in matrices.items():
        ...
        for i, (val, vec) in enumerate(zip(eigenvalues, eigenvectors.T)):
            ...
            right_side = val * vec
            print(f"Verification A|v{i+1}⟩ = λ{i+1}|v{i+1}⟩: {np.allclose(left_side, right_side)}")
        print()
...
def visualize_matrix_operations():
    ...
    for name, gate in gates.items():
        ...
        # Plot as arrows
        ax.arrow(0, 0, np.real(transformed[0]), np.imag(transformed[0]), 
                head_width=0.05, head_length=0.05, fc='red', alpha=0.7,
                length_includes_head=True)
        ax.arrow(0, 0, np.real(transformed[1]), np.imag(transformed[1]), 
                head_width=0.05, head_length=0.05, fc='blue', alpha=0.7,
                length_includes_head=True)
        
        ax.text(np.real(transformed[0]), np.imag(transformed[0]), f'{name}₀', 
               fontsize=10, ha='center')
        ax.text(np.real(transformed[1]), np.imag(transformed[1]), f'{name}₁', 
               fontsize=10, ha='center')
    ...
    for i in range(3):
        for j in range(3):
            ax.text(j, i, f'{commutation_matrix[i,j]:.1f}', 
                   ha='center', va='center', color='white' if commutation_matrix[i,j] > 1 else 'black')
    ...
    for bar, error in zip(bars, unitarity_errors):
        ...
        ax.text(bar.get_x() + bar.get_width()/2., height,
               f'{error:.2e}', ha='center', va='bottom', rotation=90)
    ...
    ax.bar(x + width/2, np.real(determinants), width, label='Determinant (real)', alpha=0.7)
...
def demonstrate_quantum_state_evolution():
    ...
    ax1.plot(steps, np.real(amplitudes_0), 'ro-', label='Real(α₀)', linewidth=2, markersize=8)
    ax1.plot(steps, np.imag(amplitudes_0), 'r--', label='Imag(α₀)', linewidth=2)
    ax1.plot(steps, np.real(amplitudes_1), 'bo-', label='Real(α₁)', linewidth=2, markersize=8)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Linear Algebra for Quantum Computing Demo')
