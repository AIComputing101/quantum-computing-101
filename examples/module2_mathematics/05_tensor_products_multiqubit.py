...
class TensorProductAnalyzer:
    ...
    def _check_separability(self, state_vector):
        ...
        if schmidt_rank > 1:
            ...
            entanglement = -np.sum(schmidt_coeffs**2 * np.log2(schmidt_coeffs**2 + 1e-12))
        else:
            entanglement = 0.0
    ...
    def three_qubit_systems(self):
        ...
        ghz_state = (three_qubit_computational['|000⟩'] + three_qubit_computational['|111⟩']) / np.sqrt(2)
        print(f"   |GHZ⟩ = (|000⟩ + |111⟩)/√2 = {ghz_state}")
        
        # W state (symmetric entangled state)
        print("\n3. W State:")
        w_state = (three_qubit_computational['|001⟩'] + 
                  three_qubit_computational['|010⟩'] + 
                  three_qubit_computational['|100⟩']) / np.sqrt(3)
    ...
    def partial_trace_demonstration(self):
        ...
        try:
            ...
            rho_1_qiskit = partial_trace(rho_qiskit, [1])  # Trace out qubit 1 (0-indexed)
    ...
    def visualize_tensor_products(self, states_dict):
        ...
        max_len = max(len(sep_analysis['schmidt_coefficients']), 
                     len(ent_analysis['schmidt_coefficients']))
        
        sep_coeffs = list(sep_analysis['schmidt_coefficients']) + [0] * (max_len - len(sep_analysis['schmidt_coefficients']))
        ent_coeffs = list(ent_analysis['schmidt_coefficients']) + [0] * (max_len - len(ent_analysis['schmidt_coefficients']))
