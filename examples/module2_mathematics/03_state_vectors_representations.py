...
class QuantumStateAnalyzer:
    ...
    def _analyze_single_state(self, state):
        ...
        print(f"  Bloch coordinates: (x={bloch_coords[0]:.4f}, y={bloch_coords[1]:.4f}, z={bloch_coords[2]:.4f})")
        
        if self.verbose:
            # Phase information
            if abs(vector[1]) > 1e-10:  # Avoid division by zero
                relative_phase = np.angle(vector[1]/vector[0]) if abs(vector[0]) > 1e-10 else np.angle(vector[1])
                print(f"  Relative Phase: {relative_phase:.4f} rad ({np.degrees(relative_phase):.2f}°)")
    ...
    def demonstrate_state_representations(self):
        ...
        print("\n3. Bloch Sphere Representation:")
        bloch_coords = self._state_to_bloch(vector)
        print(f"   r⃗ = ({bloch_coords[0]:.4f}, {bloch_coords[1]:.4f}, {bloch_coords[2]:.4f})")
    ...
    def visualize_bloch_sphere(self, states_dict):
        ...
        for i, (name, state) in enumerate(states_dict.items()):
            if len(state.data) == 2:  # Single-qubit states only
                ...
                ax.plot([0, x], [0, y], [0, z], color=colors[i % len(colors)], alpha=0.7)
    ...
    def visualize_state_evolution(self):
        ...
        for i, (name, state) in enumerate(states_evolution):
            if i < 4:  # Only plot first 4 states
                ...
                width = 0.35
                ax.bar([x - width/2 for x in x_pos], amplitudes_real, width, 
                       label='Real', alpha=0.7, color='blue')
                ax.bar([x + width/2 for x in x_pos], amplitudes_imag, width,
...
Examples:
  ...
  python 03_state_vectors_representations.py --show-evolution
    ...
    parser.add_argument('--show-bloch', action='store_true',
                       help='Display Bloch sphere visualization')
    ...
    try:
        ...
        state, density_matrix, bloch_coords = analyzer.demonstrate_state_representations()
        ...
        print(f"📊 Analyzed {len(single_qubit_states)} single-qubit and {len(multi_qubit_states)} two-qubit states")
