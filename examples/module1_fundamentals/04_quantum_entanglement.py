...
def create_bell_states():
    ...
    for label, circuit in bell_states.items():
        ...
        print(f"  Amplitudes: |00⟩:{state[0]:.3f}, |01⟩:{state[1]:.3f}, |10⟩:{state[2]:.3f}, |11⟩:{state[3]:.3f}")
        print()
...
def demonstrate_measurement_basis_effects():
    ...
    qc_zz.add_register(base_circuit.cregs[0] if base_circuit.cregs else base_circuit.add_register('c', 2)[0])
    qc_zz.measure_all()
    ...
    qc_xx.add_register(base_circuit.cregs[0] if base_circuit.cregs else base_circuit.add_register('c', 2)[0])
    qc_xx.measure_all()
    ...
    qc_zx.add_register(base_circuit.cregs[0] if base_circuit.cregs else base_circuit.add_register('c', 2)[0])
    qc_zx.measure_all()
    ...
    plt.tight_layout()
    plt.savefig('module1_04_entanglement_measurements.png', dpi=300, bbox_inches='tight')
    plt.show()
