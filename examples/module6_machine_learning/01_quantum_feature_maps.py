...
class QuantumFeatureMaps:
    ...
    def compare_feature_maps(self, X, y, test_size=0.3):
        ...
        for name, feature_map_func in feature_maps.items():
            ...
            try:
                ...
                K_train = self.compute_kernel_matrix(X_train, feature_map_func, shots=512)
    ...
    def visualize_feature_maps(self, comparison_results, expressivity_results=None):
        ...
        if expressivity_results:
            ...
            ax3.text(0.05, 0.95, f'Expressivity: {expressivity:.3f}', 
        ...
        if len(test_accs) >= 3:  # Ensure we have quantum methods
            ...
            ax5.scatter(complexities, quantum_accs, s=100, alpha=0.7, c=range(len(quantum_accs)), cmap='viridis')
            
            for i, method in enumerate(quantum_methods):
                if i < len(complexities):
                    ax5.annotate(method, (complexities[i], quantum_accs[i]), 
...
def main():
    ...
    parser.add_argument('--n-samples', type=int, default=100, help='Number of samples for random dataset')
    ...
    try:
        ...
        for method, results in comparison_results.items():
            if 'error' not in results:
                print(f"   {method:15s}: Train={results['train_accuracy']:.3f}, "
        ...
        if args.test_expressivity:
            ...
            print(f"   Expressivity measure: {expressivity_results['expressivity_measure']:.4f}")
        ...
        quantum_methods = {k: v for k, v in comparison_results.items() if k != 'Classical RBF'}
        if quantum_methods:
            best_quantum = max(quantum_methods.items(), 
                             key=lambda x: x[1].get('test_accuracy', 0))
            classical_acc = comparison_results['Classical RBF']['test_accuracy']
            
            print(f"\n🏆 Best Results:")
            print(f"   Best Quantum: {best_quantum[0]} ({best_quantum[1]['test_accuracy']:.3f})")
            print(f"   Classical RBF: {classical_acc:.3f}")
            
            if best_quantum[1]['test_accuracy'] > classical_acc:
                print(f"   🎉 Quantum advantage: +{best_quantum[1]['test_accuracy'] - classical_acc:.3f}")
            else:
                print(f"   📊 Classical leads by: +{classical_acc - best_quantum[1]['test_accuracy']:.3f}")
        
        if args.show_visualization:
            feature_maps.visualize_feature_maps(comparison_results, expressivity_results)
