...
try:
    ...
    def load_boston():
        print(
            "ℹ️  Boston housing dataset removed from sklearn, using synthetic regression data"
        )
    ...
    def fit(self, X, y, max_iter=100, learning_rate=0.01):
        ...
        for iteration in range(max_iter):
            ...
            if self.verbose and iteration % 10 == 0:
                print(
                    f"Iter {iteration}: Loss = {loss:.4f}, {metric_name} = {metric:.4f}"
                )
...
class HybridNeuralNetwork:
    ...
    def __init__(
        self, n_qubits, classical_hidden_units=10, task_type="regression", verbose=False
    ):
        self.n_qubits = n_qubits
    ...
    def fit(self, X, y):
        ...
        quantum_features = self.quantum_layer.forward_pass(
            X, self.quantum_layer.parameters
        )
...
class QNNAnalyzer:
    ...
    def compare_architectures(
        self, X_train, y_train, X_test, y_test, task_type="regression"
    ):
        ...
        try:
            ...
            results["Hybrid"] = {"score": hybrid_score, "score_name": score_name}
        ...
        results["Classical"] = {"score": classical_score, "score_name": score_name}
    ...
    def analyze_gradient_flow(self, X, y, n_qubits=2, task_type="regression"):
        ...
        qnn = QuantumNeuralNetwork(
            n_qubits, n_layers=1, task_type=task_type, verbose=False
        )
    ...
    def visualize_results(self, architecture_results, gradient_analysis=None):
        ...
        if (
            "Pure Quantum" in architecture_results
            and "training_history" in architecture_results["Pure Quantum"]
        ):
            ...
            line2 = ax2_twin.plot(
                iterations, metrics, "r-", label=metric_name, linewidth=2
            )
        ...
        if len(architectures) >= 2:
            ...
            performance_data = {
                ...
                "Hybrid": (
                    norm_scores[1]
                    if len(norm_scores) > 1 and "Hybrid" in architectures
                    else 0.7
                ),
        ...
        if architectures:
            best_idx = (
                np.argmax(scores) if score_name == "Accuracy" else np.argmin(scores)
            )
    ...
    try:
        ...
        if args.task == "regression":
            if args.dataset == "boston":
                ...
                X, y = make_regression(
                    n_samples=args.n_samples, n_features=4, noise=0.1, random_state=42
                )
            else:
                X, y = make_regression(
                    n_samples=args.n_samples, n_features=3, noise=0.1, random_state=42
                )
        else:
        ...
        X_train_q = (
            X_train[:, : args.n_qubits] if X_train.shape[1] > args.n_qubits else X_train
        )
        X_test_q = (
            X_test[:, : args.n_qubits] if X_test.shape[1] > args.n_qubits else X_test
        )
        ...
        for arch, results in architecture_results.items():
            if "error" not in results:
                print(
                    f"   {arch:15s}: {results['score_name']} = {results['score']:.3f}"
                )
            else:
        ...
        # Find best method
        valid_results = {
            k: v for k, v in architecture_results.items() if "error" not in v
        }
        if valid_results:
            if args.task == "regression":
