...
class VariationalQuantumClassifier:
    ...
    def cost_function(self, parameters, X, y):
        ...
        if self.verbose and len(self.training_history) % 10 == 0:
            print(f"Iteration {len(self.training_history)}: Loss = {loss:.4f}, Accuracy = {accuracy:.4f}")
    ...
    def fit(self, X, y, max_iter=100):
        ...
        n_params = 2 * self.n_qubits * self.depth  # 2 rotation gates per qubit per layer
...
class VQCAnalyzer:
    ...
    def quantum_vs_classical_comparison(self, X_train, y_train, X_test, y_test):
        ...
        results['Quantum VQC'] = {
            ...
            'n_parameters': 2 * 2 * 2  # 2 qubits, depth 2, 2 params per qubit per layer
        }
    ...
    def visualize_results(self, architecture_results, landscape_results=None, 
        ...
        train_accs = [architecture_results[arch].get('train_accuracy', 0) for arch in architectures]
        test_accs = [architecture_results[arch].get('test_accuracy', 0) for arch in architectures]
        
        x = np.arange(len(architectures))
        width = 0.35
        
        ax1.bar(x - width/2, train_accs, width, label='Train', alpha=0.7, color='blue')
        ...
        param_counts = [architecture_results[arch].get('n_parameters', 0) for arch in architectures]
        
        # Filter out errors
        valid_indices = [i for i, arch in enumerate(architectures) 
        ...
        if valid_param_counts:
            ax3.scatter(valid_param_counts, valid_test_accs, s=100, alpha=0.7, c=range(len(valid_param_counts)), cmap='viridis')
            
            for i, arch in enumerate(valid_architectures):
                ax3.annotate(arch, (valid_param_counts[i], valid_test_accs[i]), 
        ...
        if landscape_results:
            ...
            ax4.axvline(landscape_results['min_loss'], color='red', linestyle='--', 
                       label=f'Min: {landscape_results["min_loss"]:.3f}')
            ax4.axvline(landscape_results['mean_loss'], color='blue', linestyle='--', 
        ...
        if quantum_classical_results:
            ...
            qc_train_accs = [quantum_classical_results[m]['train_accuracy'] for m in methods]
            qc_test_accs = [quantum_classical_results[m]['test_accuracy'] for m in methods]
            
            x = np.arange(len(methods))
            ax5.bar(x - width/2, qc_train_accs, width, label='Train', alpha=0.7, color='blue')
            ax5.bar(x + width/2, qc_test_accs, width, label='Test', alpha=0.7, color='red')
...
def main():
    ...
    parser.add_argument('--dataset', choices=['iris', 'moons', 'random'], default='moons')
        ...
        if args.compare_architectures:
            ...
            for arch, results in architecture_results.items():
                if 'error' not in results:
                    print(f"   {arch:20s}: Test = {results['test_accuracy']:.3f}, "
