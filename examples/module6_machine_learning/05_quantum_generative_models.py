...
class QuantumBornMachine:
    ...
    def mmd_loss(self, target_distribution, generated_samples):
        ...
        # Target histogram
        target_hist, _ = np.histogram(target_distribution, bins=n_bins, 
                                    range=(0, n_bins), density=True)
        
        # Generated histogram
        gen_hist, _ = np.histogram(generated_samples, bins=n_bins, 
                                 range=(0, n_bins), density=True)
    ...
    def fit(self, target_data, max_iter=100, learning_rate=0.01):
        ...
        n_bins = 2 ** self.n_qubits
        target_hist, _ = np.histogram(target_data, bins=n_bins, 
                                    range=(0, n_bins), density=True)
...
class QuantumGAN:
    ...
    def train_step(self, real_data, batch_size=10):
        ...
        disc_loss = -np.mean(np.log(real_outputs + 1e-15)) - np.mean(np.log(1 - fake_outputs + 1e-15))
    ...
    def fit(self, real_data, max_iter=50):
        """Train quantum GAN."""
        # Initialize parameters
        n_gen_params = 4 * self.n_qubits_gen * 2  # 2 layers, 2 params per qubit per layer
        ...
        for iteration in range(max_iter):
            ...
            if self.verbose and iteration % 10 == 0:
                print(f"Iteration {iteration}: D_loss = {disc_loss:.4f}, G_loss = {gen_loss:.4f}")
...
class GenerativeModelAnalyzer:
    ...
    def compare_models(self, target_data, n_samples=1000):
        ...
        n_qubits = 3
        n_bins = 2 ** n_qubits
        target_discrete = np.digitize(target_data.flatten(), 
        ...
        # Generate samples
        born_samples, born_probs = born_machine.generate_samples(born_machine.parameters, n_samples)
        
        # Convert back to continuous
        born_continuous = np.interp(born_samples, 
                                  range(n_bins), 
        ...
        results['Quantum Born Machine'] = {
            ...
            'final_loss': born_machine.training_history[-1]['loss'] if born_machine.training_history else 0
        }
        ...
        # Denormalize
        gan_samples_denorm = scaler.inverse_transform(gan_samples.reshape(-1, 1)).flatten()
        
        results['Quantum GAN'] = {
            'samples': gan_samples_denorm,
            'training_history': qgan.training_history,
            'final_disc_loss': qgan.training_history[-1]['disc_loss'] if qgan.training_history else 0,
            'final_gen_loss': qgan.training_history[-1]['gen_loss'] if qgan.training_history else 0
        }
    ...
    def evaluate_quality(self, target_data, generated_samples):
        ...
        try:
            wd = wasserstein_distance(target_data.flatten(), generated_samples.flatten())
    ...
    def visualize_results(self, target_data, comparison_results):
        ...
        ax1.hist(target_data.flatten(), bins=30, alpha=0.7, label='Target', 
                color='blue', density=True, edgecolor='black')
        
        for model_name, results in comparison_results.items():
            if 'samples' in results:
                ax1.hist(results['samples'], bins=30, alpha=0.5, 
        ...
        if 'Quantum GAN' in comparison_results:
            ...
            if history:
                ...
                ax3.plot(iterations, disc_losses, 'r-', linewidth=2, label='Discriminator')
        ...
        if model_names:
            ...
            bars = ax5.bar(models, means, alpha=0.7, color=['red', 'green', 'blue'][:len(models)])
            ax5.axhline(y=target_mean, color='black', linestyle='--', 
...
def main():
    ...
    parser.add_argument('--dataset', choices=['moons', 'circles', 'blobs', 'gaussian'], 
                       default='gaussian')
    ...
    try:
        ...
        if args.dataset == 'moons':
            ...
            X, _ = make_blobs(n_samples=args.n_samples, centers=2, cluster_std=1.0, random_state=42)
            target_data = X.flatten()
        else:  # gaussian
        ...
        comparison_results = analyzer.compare_models(target_data, n_samples=args.n_samples)
        ...
        for model_name, results in comparison_results.items():
            if 'samples' in results:
                ...
                if 'final_disc_loss' in results and 'final_gen_loss' in results:
                    print(f"     Final discriminator loss: {results['final_disc_loss']:.4f}")
        ...
        if 'Quantum Born Machine' in comparison_results:
            born_history = comparison_results['Quantum Born Machine']['training_history']
            if born_history:
