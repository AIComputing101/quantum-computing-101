...
class MultiFrameworkComparator:
    ...
    def framework_feature_comparison(self):
        ...
        for feature in feature_names:
            qiskit_val = features['qiskit'][feature] if frameworks_available['qiskit'] else 'N/A'
            cirq_val = features['cirq'][feature] if frameworks_available['cirq'] else 'N/A'
            pennylane_val = features['pennylane'][feature] if frameworks_available['pennylane'] else 'N/A'
    ...
    def visualize_comparison_results(self, execution_results):
        ...
        ax1.bar(frameworks, times, alpha=0.7, color=['skyblue', 'lightcoral', 'lightgreen'][:len(frameworks)])
        ...
        for i, time_val in enumerate(times):
            ax1.text(i, time_val + max(times)*0.01, f'{time_val:.4f}s', 
                    ha='center', va='bottom')
        ...
        if len(frameworks) >= 2:
            ...
            ax2.bar([i - width/2 for i in indices], np.real(ref_state), width,
                   label=frameworks[0], alpha=0.7)
            ax2.bar([i + width/2 for i in indices], np.real(comp_state), width,
                   label=frameworks[1], alpha=0.7)
        ...
        available = [1 if frameworks_available[fw] else 0 for fw in ['qiskit', 'cirq', 'pennylane']]
    ...
    def generate_summary_report(self):
        ...
        available_frameworks = [fw for fw, available in frameworks_available.items() if available]
...
Examples:
  ...
  python 02_multi_framework_comparison.py --show-visualization
    ...
    parser.add_argument('--framework', choices=['qiskit', 'cirq', 'pennylane', 'all'],
                       default='all', help='Focus on specific framework')
