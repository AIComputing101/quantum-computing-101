...
class QuantumCircuitPatterns:
    ...
    def advanced_circuit_patterns(self):
        ...
        error_correction = QuantumCircuit(7, 3)  # 3 data + 4 ancilla qubits, 3 syndrome bits
    ...
    def circuit_optimization_patterns(self):
        ...
        print(f"   Basis gates only: {'Yes' if all(gate.name in ['rx', 'ry', 'rz', 'cx'] for gate in decomposed.data) else 'No'}")
        ...
        print(f"   Depth reduction: {(deep_circuit.depth() - shallow_circuit.depth())/deep_circuit.depth()*100:.1f}%")
        ...
        print(f"   Gate reduction: {(len(redundant.data) - len(optimized.data))/len(redundant.data)*100:.1f}%")
        ...
        for name, circuit in circuits_dict.items():
            if hasattr(circuit, 'depth'):
                metrics = {
                    ...
                    'single_qubit_gates': sum(1 for gate in circuit.data if len(gate.qubits) == 1),
                    'two_qubit_gates': sum(1 for gate in circuit.data if len(gate.qubits) == 2),
                    'parameters': len(circuit.parameters) if hasattr(circuit, 'parameters') else 0
                }
    ...
    def visualize_circuit_patterns(self, circuits_dict):
        ...
        visualizable_circuits = {name: circuit for name, circuit in circuits_dict.items() 
                               if hasattr(circuit, 'depth') and circuit.num_qubits <= 4 and circuit.depth() <= 20}
        ...
        # 4. Qubit utilization
        qubit_counts = [circuit.num_qubits for circuit in visualizable_circuits.values()]
...
Examples:
  ...
  python 03_quantum_circuit_patterns.py --pattern-type basic
    ...
    parser.add_argument('--pattern-type', 
                       ...
                       default='all', help='Focus on specific pattern type')
