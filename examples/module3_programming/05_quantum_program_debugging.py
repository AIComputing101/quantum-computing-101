...
class QuantumDebugger:
    ...
    def validate_circuit(self, circuit):
        ...
        try:
            ...
            if has_measurements and circuit.num_clbits == 0:
                validation_results['errors'].append("Circuit has measurements but no classical bits")
                validation_results['valid'] = False
            
            # 4. Very deep circuits (potential performance issue)
            if circuit.depth() > 100:
                validation_results['warnings'].append(f"Very deep circuit (depth={circuit.depth()})")
            
            # 5. High gate count
            if circuit.size() > 1000:
                validation_results['warnings'].append(f"High gate count ({circuit.size()})")
            ...
            if circuit.parameters:
                validation_results['warnings'].append(f"Circuit has {len(circuit.parameters)} unbound parameters")
            
            self.log_debug(f"Circuit validation completed. Valid: {validation_results['valid']}")
        ...
        for i, instruction in enumerate(circuit.data):
            ...
            if gate_name == 'x' and len(qubits) == 1:
                ...
                if qubit in gate_history and gate_history[qubit][-1][0] == 'x':
                    redundancies.append(f"Double X gate on qubit {qubit} at positions {gate_history[qubit][-1][1]}, {i}")
    ...
    def trace_execution(self, circuit, steps=None):
        ...
        try:
                ...
                if instruction.operation.name != 'measure':
                    try:
                        ...
                        step_info = {
                            ...
                            'entanglement': self._calculate_entanglement(state.data, circuit.num_qubits)
                        }
                        
                        trace_results['steps'].append(step_info)
                        trace_results['intermediate_states'].append(state.data.copy())
                        
                        self.log_debug(f"Step {step}: {instruction.operation.name} on qubits {step_info['qubits']}")
                        
                    except Exception as e:
                        self.log_debug(f"Could not compute state at step {step}: {str(e)}", level='WARNING')
                        break
                else:
                    step_info = {
                        'step': step,
            ...
            self.log_debug(f"Execution trace completed for {len(trace_results['steps'])} steps")
    ...
    def error_detection(self, circuit, expected_state=None):
        ...
        if circuit.depth() > 50:
            errors['performance_issues'].append(f"Deep circuit may suffer from decoherence (depth={circuit.depth()})")
        
        if circuit.count_ops().get('cx', 0) > circuit.num_qubits * 5:
            errors['performance_issues'].append("High number of two-qubit gates may reduce fidelity")
        ...
        if expected_state is not None:
            try:
                ...
                if fidelity < 0.99:
                    errors['quantum_errors'].append(f"State fidelity below threshold: {fidelity:.4f}")
                
            except Exception as e:
                errors['quantum_errors'].append(f"Could not verify state: {str(e)}")
        
        self.log_debug(f"Error detection completed. Found {sum(len(v) for v in errors.values())} issues")
        return errors
    
    def generate_debug_report(self, circuit, trace_results=None, performance_results=None):
        ...
        if performance_results:
            print(f"\n⚡ Performance Summary:")
            print(f"   Transpilation time: {performance_results['transpilation_time']:.4f}s")
    ...
    def visualize_debugging_results(self, trace_results, performance_results=None):
        ...
        if prob_evolution:
            ...
            for i in range(min(4, prob_array.shape[1])):  # Show first 4 basis states
                axes[0, 0].plot(steps, prob_array[:, i], label=f'|{i:0{int(np.log2(prob_array.shape[1]))}b}⟩', marker='o')
        ...
        if entanglement_values:
            axes[0, 1].plot(steps[:len(entanglement_values)], entanglement_values, 'r-o')
        ...
        if performance_results:
            ...
            axes[1, 1].bar(x + width/2, transpiled, width, label='Transpiled', alpha=0.7)
...
Examples:
  ...
  python 05_quantum_program_debugging.py --trace-steps 5
    ...
    parser.add_argument('--example', 
                       ...
                       default='bell-state', help='Example circuit to debug')
    parser.add_argument('--trace-steps', type=int, default=None,
    ...
    parser.add_argument('--profile-backend', action='store_true',
                       help='Include backend profiling')
    ...
    try:
        ...
        print(f"Circuit: {circuit.num_qubits} qubits, {circuit.depth()} depth, {circuit.size()} gates")
