...
class HardwareOptimizer:
    ...
    def create_mock_backends(self):
        ...
        grid_coupling = [(0, 1), (1, 2), (2, 3), (3, 4), (0, 5), (1, 6), (2, 7), (3, 8), (4, 9),
                        (5, 6), (6, 7), (7, 8), (8, 9)]
        backends['grid_10q'] = {
            'name': 'Grid 10-qubit',
            'n_qubits': 10,
        ...
        hex_coupling = [(0, 1), (1, 2), (2, 3), (1, 4), (4, 7), (7, 10), (10, 12),
                       (2, 5), (5, 8), (8, 11), (11, 14), (3, 6), (6, 9), (9, 13),
                       (13, 15), (4, 5), (5, 6), (7, 8), (8, 9), (10, 11), (11, 12),
                       (12, 13), (13, 14)]
    ...
    def optimize_for_backend(self, circuit, backend_name, optimization_level=3):
        ...
        for strategy_name, transpile_options in strategies.items():
            try:
                ...
                depth_reduction = (original_analysis['depth'] - transpiled_analysis['depth']) / original_analysis['depth']
                ...
                gate_overhead = (gate_count_transpiled - gate_count_original) / gate_count_original
                ...
                if self.verbose:
                    print(f"   {strategy_name}: depth {transpiled_analysis['depth']} "
        ...
        # Find best strategy
        successful_strategies = {k: v for k, v in optimization_results.items() if v['success']}
        ...
        if successful_strategies:
            ...
            optimization_results['best_circuit'] = successful_strategies[best_strategy]['circuit']
    ...
    def compare_backend_suitability(self, circuit):
        ...
        for backend_name, backend_info in self.backends.items():
            ...
            available_connections = set(tuple(sorted(edge)) for edge in backend_info['coupling_map'])
            ...
            if circuit_gates.issubset(backend_gates):
                ...
                details['gate_set'] = f'{len(non_native_gates)} gates need decomposition'
        ...
        # Rank backends
        ranked_backends = sorted(suitability_scores.items(), 
    ...
    def estimate_circuit_fidelity(self, circuit, backend_info):
        ...
        for t1, t2 in zip(t1_times, t2_times):
            ...
            coherence_fidelity *= np.exp(-circuit_time / t1) * np.exp(-circuit_time / t2)
        ...
        for circuit_name, circuit in circuits.items():
            ...
            for backend_name in backends.keys():
                ...
                try:
                    ...
                    calibration_result = self.optimizer.create_calibration_aware_optimization(
            ...
            for backend_name, results in circuit_results.items():
                if results['success'] and 'optimization' in results:
                    ...
                    for strategy, strategy_result in opt_result.items():
                        if isinstance(strategy_result, dict) and 'depth_reduction' in strategy_result:
                            if strategy not in strategy_performance:
                                strategy_performance[strategy] = []
                            strategy_performance[strategy].append(strategy_result['depth_reduction'])
...
def visualize_optimization_results(optimizer, benchmark_results, analysis):
    ...
    for i, (backend_name, backend_info) in enumerate(optimizer.backends.items()):
        if i < 2:  # Show first two backends
            ...
            # Plot
            nx.draw(G, pos, ax=ax1, node_color=f'C{i}', 
                   node_size=200, alpha=0.7, label=backend_name)
    ...
    if 'strategy_performance' in analysis:
        ...
        if strategy_names:
            bars = ax4.bar(strategy_names, strategy_scores, alpha=0.7, color='lightgreen')
    ...
    for i, circuit_name in enumerate(circuits):
        for j, backend_name in enumerate(backends):
            if (circuit_name in benchmark_results and 
                backend_name in benchmark_results[circuit_name] and
                benchmark_results[circuit_name][backend_name]['success']):
                
                # Use fidelity as metric
                fidelity = benchmark_results[circuit_name][backend_name]['calibration']['expected_fidelity']
...
def main():
    ...
    parser.add_argument('--circuit', choices=['qft', 'quantum_volume', 'random_heavy', 'bell_state', 'ghz_state'], 
                       default='qft', help='Circuit to optimize')
    parser.add_argument('--backend', choices=['linear_5q', 'grid_10q', 'heavy_hex_16q', 'all_to_all_5q'],
                       default='linear_5q', help='Target backend')
    parser.add_argument('--optimization-level', type=int, default=3, 
                       help='Transpiler optimization level')
    parser.add_argument('--compare-backends', action='store_true',
                       help='Compare backend suitability')
    parser.add_argument('--run-benchmark', action='store_true',
                       help='Run full optimization benchmark')
    ...
    try:
        ...
        for name, backend in backends.items():
            print(f"   {name}: {backend['n_qubits']} qubits, "
        ...
        if not args.run_benchmark and not args.compare_backends:
            ...
            if 'best_strategy' in optimization_result:
                ...
                print(f"   Optimized gates: {sum(dict(best_circuit.count_ops()).values())}")
                ...
                for strategy, result in optimization_result.items():
                    if isinstance(result, dict) and 'success' in result and result['success']:
                        print(f"     {strategy}: depth {result['analysis']['depth']}, "
                              f"reduction {result['depth_reduction']:.1%}")
            
            # Calibration-aware optimization
            print(f"\n🎛️  Calibration-aware optimization:")
            cal_result = optimizer.create_calibration_aware_optimization(circuit, args.backend)
            ...
            print(f"   Depth change: {summary['original_depth']} → {summary['optimized_depth']}")
            print(f"   Gate change: {summary['original_gates']} → {summary['optimized_gates']}")
        ...
        if args.compare_backends:
            ...
            suitability_scores, circuit_analysis = optimizer.compare_backend_suitability(circuit)
        ...
        if args.run_benchmark:
            ...
            for circuit_name, circuit_results in benchmark_results.items():
                ...
                successful_backends = [name for name, result in circuit_results.items() 
                                     if result['success']]
                print(f"     Successful backends: {len(successful_backends)}/{len(circuit_results)}")
                ...
                if circuit_name in analysis['best_backend_per_circuit']:
                    ...
                    if best_backend:
                        ...
                        if best_backend in circuit_results:
                            fidelity = circuit_results[best_backend]['calibration']['expected_fidelity']
            ...
            if 'best_strategy_overall' in analysis:
                ...
                if 'strategy_performance' in analysis:
                    ...
                    for strategy, improvements in analysis['strategy_performance'].items():
                        avg_improvement = np.mean(improvements)
                        print(f"     {strategy}: {avg_improvement:.1%} avg depth reduction")
        ...
        print(f"   • Circuit-backend matching can improve results by orders of magnitude")
