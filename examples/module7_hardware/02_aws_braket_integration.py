                ...
                for arn in simulator_arns:
                    try:
                        ...
                        device_info = {
                            ...
                            'properties': device.properties.dict() if hasattr(device, 'properties') else {}
                        }
    ...
    def submit_task(self, circuit, device_arn, shots=1000):
        ...
        try:
            if device_arn == 'local:simulator':
                ...
                task = device.run(circuit, shots=shots, s3_destination_folder=(self.s3_bucket, "braket-results"))
        ...
        try:
            if device_arn == 'local:simulator':
                ...
                capabilities = {
                    ...
                    'status': device.status,
                    'properties': properties.dict() if hasattr(properties, 'dict') else str(properties)
                }
...
class BraketAnalyzer:
    ...
    def compare_providers(self, test_circuits=None):
        ...
        for device in available_devices[:3]:  # Limit to avoid long execution
            ...
            for i, circuit in enumerate(test_circuits):
                ...
                if braket_circuit:
                    start_time = time.time()
                    task_result = self.manager.submit_task(braket_circuit, device_arn, shots=100)
                    ...
                    if task_result['success'] and 'result' in task_result:
                        ...
                        circuit_result['error'] = task_result.get('error', 'Unknown error')
    ...
    def visualize_results(self, provider_results, cost_analysis=None):
        ...
        if device_names:
            colors = ['green' if sr >= 0.8 else 'orange' if sr >= 0.5 else 'red' 
        ...
        if qubit_counts:
            ...
            simulator_qubits = [q for q, t in zip(qubit_counts, device_types) if t == 'SIMULATOR']
            hardware_qubits = [q for q, t in zip(qubit_counts, device_types) if t == 'QPU']
        ...
        if provider_results:
            ...
            successful_devices = sum(1 for results in provider_results.values() 
...
def main():
    ...
    parser.add_argument('--device-arn', type=str, default='local:simulator',
                       help='Device ARN to use')
    ...
    parser.add_argument('--compare-providers', action='store_true',
                       help='Compare different providers')
    ...
    try:
            ...
            for provider, provider_devices in manager.providers.items():
                if provider_devices:
                    ...
                    for device in provider_devices:
                        status_icon = "🟢" if device.get('status') == 'AVAILABLE' else "🔴"
                        qubit_info = f" ({device.get('properties', {}).get('qubit_count', '?')} qubits)" if 'properties' in device else ""
        ...
        if args.compare_providers:
            ...
            for device_name, results in provider_results.items():
                ...
                if successful > 0:
                    avg_time = np.mean([r['execution_time'] for r in circuit_results if r['success']])
            ...
            if args.show_costs:
                ...
                for device_name, cost_info in cost_analysis.items():
                    ...
                    print(f"     Estimated total cost: ${cost_info['estimated_cost']:.3f}")
                    print(f"     Throughput: {cost_info['shots_per_second']:.1f} shots/s")
        ...
        if not args.compare_providers:
            ...
            if braket_circuit:
                ...
                # Submit task
                task_result = manager.submit_task(braket_circuit, args.device_arn, shots=args.shots)
